...
\normalsize (convención de signo alineada con las regresiones: $\GaR$ como variable primitiva)}
\author{}
\date{\today}
...
\begin{abstract}
\noindent Este apéndice detalla, paso a paso, cada derivada y conclusión matemática del modelo. \textbf{Cambio de convención respecto de la versión anterior:} el bloque soberano se reescribe usando el \emph{Growth-at-Risk} $\GaR_\tau$ como variable primitiva ---el cuantil bajo del crecimiento, negativo en la cola--- \emph{en lugar} de su magnitud $D=-\GaR$. Con ello los signos derivados del modelo coinciden \emph{directamente} con los de las regresiones, sin ningún cambio de variable intermedio: $\beta_1>0$, $\beta_2<0$, $\beta_3<0$, $\beta_4<0$. Todas las identidades (incluidas la derivada cruzada y la tercera derivada) fueron verificadas numéricamente (diferencias finitas frente a fórmula analítica; coincidencia hasta $10^{-6}$).
\end{abstract}
...
\beta_2=\frac{\partial \EMBI}{\partial \GaR}<0,\quad
\beta_3=\frac{\partial^2 \EMBI}{\partial \JLoss\,\partial \GaR}<0,\quad
...
El prestatario resuelve $\max_p U(p;R_L)$, $U=(1-p)[R(p)-(1+R_L)]$, con $R'(p)>0$ y $(1-p)R(p)$ cóncava.
...
Con $\PD(n)=\Phi(\bar z(n))$, $\bar z(n)=\big[\Phi^{-1}(p^*(n))-\sqrt{1-\rho}\,\Phi^{-1}(\bar x(n))\big]/\sqrt{\rho}$, $\bar x'(R_L)=\frac{1+r_D-e}{(1+R_L)^2}>0$.
...
\end{proposition}
\begin{proof}
$\Var(L_{\text{sys}}\mid Z)=\sum_i\lambda_i^2 q(Z)(1-q(Z))=H\,q(Z)(1-q(Z))$. A igual media condicional, mayor $H$ engrosa la cola (orden convexo), y el $\ES$ es creciente en dicho orden; $\partial\ES_\alpha/\partial H>0$. En $n\to\infty$, $H\to0$ y $L_{\text{sys}}\to q(Z)$ (Vasicek).
\end{proof}

%==================================================================
\section{El bloque soberano y la derivada cruzada (convención $\GaR$)}\label{sec:soberano}

\emph{Esta es la sección reescrita.} Se usa $\GaR_\tau$ como variable primitiva. La clave técnica es que el \emph{crecimiento de cola} que entra en la dinámica de deuda es precisamente el Growth-at-Risk: $g=\GaR_\tau$.
...
\noindent\emph{Interpretación:} un $\GaR$ más alto (crecimiento de cola menos adverso) reduce la deuda proyectada y \emph{aumenta} el espacio fiscal. Es el signo \emph{opuesto} al de la versión con $D=-\GaR$: aquí la variable buena es alta cuando la economía está segura.
\end{proof}
...
\subsection{Proposición 4: la derivada cruzada es negativa ($\beta_3<0$)}

\begin{proposition}[complementariedad $\JLoss\times\GaR$]\label{prop:cross}
...
\subsection{Amplificación por concentración: $\beta_4<0$}
...
%==================================================================
\section{Mapeo a la ecuación empírica (signos alineados)}

La ecuación estimada es
\[
\EMBI_{i,t}=\alpha_i+\delta_t+\beta_1\JLoss+\beta_2\GaR+\beta_3(\JLoss\times\GaR)+\beta_4(\JLoss\times\GaR\times\HHI)+\boldsymbol\theta'\text{Int}^-+\boldsymbol\gamma'X+\varepsilon_{i,t},
\]
donde $\text{Int}^-=\{\JLoss\times\HHI,\ \GaR\times\HHI\}$. La correspondencia teórica, ahora \emph{sin cambios de variable}, es
\[
\beta_1=\frac{\partial\EMBI}{\partial\JLoss}>0,\quad
\beta_2=\frac{\partial\EMBI}{\partial\GaR}<0,\quad
\beta_3=\frac{\partial^2\EMBI}{\partial\JLoss\partial\GaR}<0\ (\text{Prop.~\ref{prop:cross}}),\quad
\beta_4=\frac{\partial^3\EMBI}{\partial\JLoss\partial\GaR\partial\HHI}<0\ (\text{Prop.~\ref{prop:amp}}).
\]
...
$\beta_2$ & $\GaR$ & $-$ & Lema~\ref{lem:first} \\
$\beta_3$ & $\JLoss\times\GaR$ & $-$ & Prop.~\ref{prop:cross} \\
$\beta_4$ & $\JLoss\times\GaR\times\HHI$ & $-$ & Prop.~\ref{prop:amp} \\
\bottomrule
\end{tabular}
\end{center}

\begin{remark}[coherencia con las regresiones]
Estos cuatro signos coinciden directamente con los estimados. En particular, $\beta_3<0$ y $\beta_4<0$ ---que parecían anómalos bajo la convención $D=-\GaR$--- son la predicción \emph{correcta} del modelo cuando la variable de la regresión es $\GaR$. El objeto firmado sigue siendo la \textbf{derivada cruzada} (y su amplificación); el vector $\boldsymbol\theta$ (interacciones dobles) es de control.
\end{remark}
