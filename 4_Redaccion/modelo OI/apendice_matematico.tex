...
\title{\textbf{Apéndice Matemático}\\[4pt]
\large Demostraciones detalladas paso a paso\\
\normalsize (convención de riesgo de cola $D\equiv-\GaR$; las derivadas se calculan en $\GaR$ por comodidad algebraica)}
\author{}
\date{\today}
...
\begin{abstract}
\noindent Este apéndice detalla, paso a paso, cada derivada y conclusión matemática del modelo. El bloque soberano se \emph{deriva} sobre el \emph{Growth-at-Risk} $\GaR_\tau$ ---el cuantil bajo del crecimiento, negativo en la cola--- por comodidad algebraica (la forma $1/(1+\GaR)$ del servicio de deuda), y el resultado final se reexpresa en el \textbf{riesgo de cola} $D\equiv-\GaR$, la convención de las regresiones. En esa convención los cuatro signos predichos son $\beta_1>0$, $\beta_2^{D}>0$, $\beta_3>0$, $\beta_4>0$, con la traducción $\beta_k^{\GaR}=-\beta_k^{D}$ para $k\in\{2,3,4\}$. Todas las identidades (incluidas la derivada cruzada y la tercera derivada) fueron verificadas numéricamente (diferencias finitas frente a fórmula analítica; coincidencia hasta $10^{-6}$).
\end{abstract}

\begin{mdframed}
\textbf{Convención de signo (léase primero).} $\GaR_\tau$ es el cuantil $\tau$ (p.ej.\ $5\%$) del crecimiento: un $\GaR$ \emph{más alto} = cola \emph{menos} adversa. El \textbf{riesgo de cola} es $D\equiv-\GaR$ ($D$ \emph{más alto} = cola más adversa), la variable de las regresiones. Variables observadas: $\JLoss$ (fragilidad sistémica, $\uparrow$ = peor), $D$ ($\uparrow$ = peor), $\HHI$ (concentración, $\uparrow$ = más concentrado), $\EMBI$ ($\uparrow$ = mayor riesgo soberano). Los cuatro coeficientes firmados, en convención $D$, son
\[
\beta_1=\frac{\partial \EMBI}{\partial \JLoss}>0,\quad
\beta_2^{D}=\frac{\partial \EMBI}{\partial D}>0,\quad
\beta_3=\frac{\partial^2 \EMBI}{\partial \JLoss\,\partial D}>0,\quad
\beta_4=\frac{\partial^3 \EMBI}{\partial \JLoss\,\partial D\,\partial \HHI}>0.
\]
Las derivadas de abajo se escriben en $\GaR$; la traducción es $\partial/\partial D=-\partial/\partial\GaR$, de modo que cada resultado ``$<0$ respecto de $\GaR$'' equivale a ``$>0$ respecto de $D$''. En convención $\GaR$, $\theta=\beta_3^{\GaR}<0$ y $\beta_4^{\GaR}<0$.

\medskip\noindent\emph{Numeración.} Este apéndice numera sus proposiciones de forma independiente (Prop.~1 = U de la fragilidad; 2 = mínimo sistémico desplazado; 3 = granularidad; 4 = derivada cruzada; 5 = amplificación). En el documento de tesis, la numeración global antepone la existencia y unicidad del equilibrio como Proposición~1, de modo que la U es la Proposición~2, etc.
\end{mdframed}
...
\begin{lemma}[derivadas de $\Phi_2$]\label{lem:phi2}
\begin{align}
...
%==================================================================
\section{El bloque bancario (sin cambios respecto de $\GaR$)}

\emph{Nota:} las Secciones \ref{sec:prelim}--\ref{sec:jl} \textbf{no dependen de la convención sobre $\GaR$}; se reproducen para completitud. Solo el bloque soberano (Sección~\ref{sec:soberano}) se ve afectado por el cambio de variable.

\subsection{Lema de \textit{risk-shifting}}\label{sec:rs}
...
\begin{lemma}[efecto pro-competitivo]\label{lem:RL}
...
\begin{proposition}[U-shape]\label{prop:U}
...
\subsection{$\JLoss$ y sus monotonías}\label{sec:jl}
...
\begin{lemma}[monotonías de $\JLoss$]\label{lem:jl}
...
\begin{proposition}[desplazamiento del mínimo]\label{prop:shift}
...
\subsection{Proposición 3: traspaso creciente en la concentración}\label{sec:gran}
Con $L_{\text{sys}}=\sum_i\lambda_i\1\{\text{quiebra}_i\}$, $H=\sum_i\lambda_i^2=1/n$, quiebras Bernoulli independientes dado $Z$.
\begin{proposition}[granularidad]\label{prop:gran}
...
%==================================================================
\section{El bloque soberano y la derivada cruzada}\label{sec:soberano}

Las derivadas se calculan en $\GaR_\tau$ por comodidad algebraica; el mapeo final
(Sección~\ref{sec:map}) las reexpresa en $D\equiv-\GaR$. La clave técnica es que el
\emph{crecimiento de cola} que entra en la dinámica de deuda es precisamente el
Growth-at-Risk: $g=\GaR_\tau$.

\subsection{Supuestos}
\begin{assumption}[estructura fiscal]\label{as:B}
$B(\JLoss;H)$ con $B_J\equiv\partial B/\partial\JLoss>0$, $\partial B/\partial H>0$, $\partial B_J/\partial H>0$; $Y>0$.
\end{assumption}
\begin{assumption}[shock fiscal]\label{as:eta}
...
\end{assumption}
\begin{assumption}[crecimiento de cola]\label{as:g}
...
\begin{equation}
DFL(\JLoss,\GaR,H)=\bar d-\underbrace{\left[\frac{1+r}{1+\GaR}\,d+\frac{B(\JLoss;H)}{Y}\right]}_{d'\ (\text{deuda proyectada})},\qquad
\EMBI=LGD\,\big(1-F_\eta(DFL)\big).
\label{eq:embi}
\end{equation}

\begin{lemma}[derivadas de $DFL$]\label{lem:dfl}
\[
...
\end{lemma}
\begin{proof}
La primera y la tercera son inmediatas. Para la segunda, con $g=\GaR$ (Supuesto~\ref{as:g}):
\[
...
\noindent\emph{Interpretación:} un $\GaR$ más alto (crecimiento de cola menos adverso) reduce la deuda proyectada y \emph{aumenta} el espacio fiscal; equivalentemente, $\partial DFL/\partial D<0$ (más riesgo de cola, menos espacio fiscal).
\end{proof}

\begin{lemma}[efectos de primer orden: $\beta_1>0$, $\beta_2<0$]\label{lem:first}
\[
...
\]
El signo de $\beta_2$ respecto de $\GaR$ es \emph{negativo}: mejor crecimiento de cola, menor spread. Equivalentemente, $\beta_2^{D}=\partial\EMBI/\partial D=-\partial\EMBI/\partial\GaR>0$: más riesgo de cola, mayor spread. $\blacksquare$
\end{proof}

\subsection{Proposición 4: la derivada cruzada ($\beta_3>0$ en convención $D$)}

\begin{proposition}[complementariedad $\JLoss\times D$]\label{prop:cross}
Bajo los Supuestos~\ref{as:B}--\ref{as:g},
\[
\boxed{\ \frac{\partial^2 \EMBI}{\partial \JLoss\,\partial \GaR}=LGD\,\frac{B_J}{Y}\,f_\eta'(DFL)\,\frac{\partial DFL}{\partial \GaR}\;<\;0
\;\Longleftrightarrow\;
\frac{\partial^2 \EMBI}{\partial \JLoss\,\partial D}\;>\;0,\quad D\equiv-\GaR.\ }
\]
\end{proposition}
\begin{proof}
\textbf{Paso 1 (punto de partida).} Del Lema~\ref{lem:first}, el efecto marginal de la fragilidad es
\[
g(\JLoss,\GaR,H)\equiv\frac{\partial \EMBI}{\partial \JLoss}=LGD\,\frac{B_J}{Y}\,f_\eta\big(DFL(\JLoss,\GaR,H)\big),
\]
y $LGD,Y,B_J$ no dependen de $\GaR$ (Supuesto~\ref{as:B}: $B$ depende de $\JLoss,H$).
...
Factor & Signo & Razón \\
\midrule
$LGD\,B_J/Y$ & $>0$ & $LGD>0$, $B_J>0$ (Sup.~\ref{as:B}), $Y>0$ \\
$f_\eta'(DFL)$ & $<0$ & $DFL>0=$ moda, cola derecha (Sup.~\ref{as:eta}) \\
$\partial DFL/\partial \GaR$ & $>0$ & Lema~\ref{lem:dfl} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{remark}[por qué esto \emph{es} la complementariedad]
En convención $D\equiv-\GaR$ el signo es \emph{positivo} y directamente intuitivo: ``más riesgo por más riesgo eleva el spread''. El efecto marginal de la fragilidad, $\partial\EMBI/\partial\JLoss=LGD f_\eta(DFL)B_J/Y>0$, es \emph{creciente} en $D$: cuando $D$ es bajo (economía segura) la fragilidad importa poco; cuando $D$ \emph{sube} (crecimiento de cola más adverso), el mismo aumento de fragilidad eleva el spread mucho más. En convención $\GaR$ el mismo hecho se escribe $\beta_3^{\GaR}<0$ (el efecto es decreciente en $\GaR$), por la razón puramente mecánica de que $\GaR$ baja en la cola. En ambas parametrizaciones, $\beta_3$ codifica que \emph{fragilidad y mal crecimiento se refuerzan}.
\end{remark}

\begin{remark}[verificación numérica]
Punto representativo ($LGD=0{,}55$, $\sigma_\eta=0{,}16$, $b_0=0{,}32$, $b_1=1{,}2$, $d=0{,}55$, $\bar d=1{,}10$, $r=0{,}04$, $\JLoss=0{,}22$, $\GaR=-0{,}05$, $H=1/6$): $DFL=0{,}413>0$; $LGD\,B_J/Y=0{,}2112$; $f_\eta'(DFL)=-1{,}4296$; $\partial DFL/\partial\GaR=+0{,}6338$. Cross-partial respecto de $\GaR$ $=-0{,}191360$ (respecto de $D$: $+0{,}191360$), idéntico (hasta $10^{-6}$) a las diferencias finitas de $\EMBI$.
\end{remark}

\subsection{Amplificación por concentración: $\beta_4>0$ en convención $D$}

\begin{proposition}[la complementariedad se intensifica con la concentración]\label{prop:amp}
Sea $C(H)\equiv\dfrac{\partial^2 \EMBI}{\partial \JLoss\,\partial \GaR}<0$. Bajo los Supuestos~\ref{as:B}--\ref{as:g} y la condición de cola profunda $f_\eta''(DFL)\ge0$,
\[
\frac{\partial C}{\partial H}=\frac{\partial^3 \EMBI}{\partial \JLoss\,\partial \GaR\,\partial H}<0
\;\Longleftrightarrow\;
\beta_4=\frac{\partial^3 \EMBI}{\partial \JLoss\,\partial D\,\partial H}>0,
\]
es decir, el término de interacción se intensifica al aumentar la concentración: su magnitud crece.
\end{proposition}
\begin{proof}
...
La concentración eleva el costo marginal del rescate $B_J$, intensificando la interacción (aumentando su magnitud). El canal (II) lo refuerza en la cola profunda. \emph{(Verificado: $DFL/\sigma_\eta=2{,}58>1\Rightarrow f_\eta''>0$; $\partial^3\EMBI/\partial\JLoss\partial\GaR\partial H=-0{,}413<0$, esto es $\beta_4=+0{,}413>0$ en convención $D$.)}
\end{remark}
...
%==================================================================
\section{Mapeo a la ecuación empírica}\label{sec:map}

La ecuación estimada, en la convención de riesgo de cola $D\equiv-\GaR$, es
\[
\EMBI_{i,t}=\alpha_i+\delta_t+\beta_1\JLoss+\beta_2^{D} D+\beta_3(\JLoss\times D)+\beta_4(\JLoss\times D\times\HHI)+\boldsymbol\varphi'\text{Int}^-+\boldsymbol\gamma'X+\varepsilon_{i,t},\qquad D\equiv-\GaR,
\]
donde $\text{Int}^-=\{\JLoss\times\HHI,\ D\times\HHI\}$. La correspondencia teórica, usando $\partial/\partial D=-\partial/\partial\GaR$, es
\[
\beta_1=\frac{\partial\EMBI}{\partial\JLoss}>0,\quad
\beta_2^{D}=\frac{\partial\EMBI}{\partial D}=-\frac{\partial\EMBI}{\partial\GaR}>0,\quad
\beta_3=\frac{\partial^2\EMBI}{\partial\JLoss\,\partial D}>0\ (\text{Prop.~\ref{prop:cross}}),\quad
\beta_4=\frac{\partial^3\EMBI}{\partial\JLoss\,\partial D\,\partial\HHI}>0\ (\text{Prop.~\ref{prop:amp}}).
\]
En la parametrización con $\GaR$ (la que usan las tablas de \texttt{bateria\_bbg.csv} y \texttt{NUMEROS\_CANONICOS\_BBG.md}) los signos son $\beta_2<0$, $\theta=\beta_3^{\GaR}<0$ y $\beta_4^{\GaR}<0$; la traducción es $\beta_k^{D}=-\beta_k^{\GaR}$.

\begin{center}
\begin{tabular}{clccc}
\toprule
Coef. & Término & Signo ($D$) & Signo ($\GaR$) & Fuente \\
\midrule
$\beta_1$ & $\JLoss$ & $+$ & $+$ & Lema~\ref{lem:first} \\
$\beta_2$ & $D=-\GaR$ & $+$ & $-$ & Lema~\ref{lem:first} \\
$\beta_3$ & $\JLoss\times D$ & $+$ & $-$ & Prop.~\ref{prop:cross} \\
$\beta_4$ & $\JLoss\times D\times\HHI$ & $+$ & $-$ & Prop.~\ref{prop:amp} \\
\bottomrule
\end{tabular}
\end{center}

\begin{remark}[coherencia con las regresiones]
En convención $D=-\GaR$ estos cuatro signos coinciden directamente con los reportados en el trabajo empírico complementario y en los \texttt{.Rmd} del panel. El objeto firmado central es la \textbf{derivada cruzada} (y su amplificación); el vector $\boldsymbol\varphi$ (interacciones dobles) es de control.
\end{remark}
...
\bibitem[Plackett(1954)]{Plackett1954} Plackett, R. (1954). A Reduction Formula for Normal Multivariate Integrals. \textit{Biometrika} 41(3/4), 351--360.
\end{thebibliography}
