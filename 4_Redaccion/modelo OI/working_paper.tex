...
\begin{abstract}
\noindent Este trabajo desarrolla un modelo teórico formal en el que la \textit{estructura de mercado bancario} es el parámetro profundo que gobierna la cadena que va desde la fragilidad sistémica hasta el riesgo soberano en economías en desarrollo. En un mercado de crédito con competencia à la Cournot, responsabilidad limitada y seguro de depósitos, la intensidad de competencia opera sobre el riesgo por dos canales de signo opuesto ---efecto margen (Keeley) y efecto \textit{risk-shifting} (Boyd--De Nicoló)---, generando una relación en U con la probabilidad de quiebra individual (Proposición~1, que anida a \citealp{MMR2010}). El aporte central es distinguir el riesgo \textit{individual} del riesgo de \textit{cola conjunta} del sistema ($\JLoss$): al endogenizar la correlación de activos como función de la estructura de mercado, la concentración no solo desplaza la probabilidad de default sino que engrosa la cola conjunta, de modo que el mínimo de $\JLoss$ se ubica en un mercado más competido que el de mínima fragilidad individual (Proposición~2), y el traspaso de la cola es creciente en la concentración (Proposición~3). El cierre macro-soberano ---un canal de \textit{credit crunch} ($\JLoss\!\to\!\GaR$) y un bloque de límite fiscal ($\GaR,\JLoss\!\to\!\EMBI$)--- produce una complementariedad: el efecto de la fragilidad sobre el spread se amplifica cuando el crecimiento ya está en la cola, y esa amplificación crece con la concentración (Proposición~4). Todas las proposiciones se ilustran numéricamente, el método de cálculo de $\JLoss$ se valida contra Monte Carlo, y el diseño empírico se valida primero por Monte Carlo (con $\GaR$ como regresor, recuperación de los signos $\beta_3,\beta_4<0$ en el 100\% de $3000$ réplicas) y luego se contrasta con datos reales de un panel de economías emergentes: la complementariedad ($\beta_3<0$, equivalentemente $\theta<0$ en la parametrización de nivel) es una regularidad robusta dentro del núcleo de economías latinoamericanas, y su amplificación por la concentración bancaria ($\beta_4<0$) se confirma en un panel de once economías emergentes usando la concentración estructural de \textit{World Bank GFDD} ($\hat\beta_4$ de signo consistente con la predicción en cuatro de cinco proxies de competencia/concentración), con la honestidad de que la magnitud es todavía imprecisa por el tamaño muestral disponible. El modelo sobrevive además a una estructura de competencia alternativa (Salop) y a una revisión adversarial explícita.
...
\paragraph{Relación con la investigación empírica complementaria.} Este trabajo teórico es la contraparte de un segundo artículo de investigación, de naturaleza empírica, elaborado en paralelo por el mismo autor, que estima directamente sobre un panel de economías emergentes latinoamericanas el término de interacción $\JLoss\times\GaR$ sobre el spread soberano y documenta su significancia y robustez (\citealp{Chari2024} ofrece el antecedente más próximo, con la interacción de $\JLoss$ y factores financieros \emph{globales}). Los dos trabajos comparten las mismas métricas de fragilidad bancaria y riesgo de cola, construidas de manera idéntica, pero difieren en su objeto: mientras el artículo empírico establece la complementariedad $\JLoss\times\GaR$ como una regularidad estadística sin comprometerse con un mecanismo estructural único, este trabajo deriva esa misma complementariedad --y, decisivamente, su heterogeneidad por la concentración bancaria-- desde primeros principios de un modelo de competencia. La Sección~\ref{sec:empirico} reporta, además de la validación del diseño por Monte Carlo, una primera puesta a prueba de las predicciones distintivas del modelo ($\beta_3$ y, sobre todo, $\beta_4$) sobre el mismo panel de datos reales.

El resto del documento se organiza así. La Sección~\ref{sec:modelo} presenta el modelo. La Sección~\ref{sec:sistemico} deriva la fragilidad individual y sistémica (Proposiciones~1--3) y la calibra. La Sección~\ref{sec:soberano} cierra la cadena macro-soberana (Proposición~4). La Sección~\ref{sec:empirico} desarrolla y valida la estrategia empírica. La Sección~\ref{sec:robustez} presenta la robustez y la revisión adversarial. La Sección~\ref{sec:conclusion} concluye.

\section{El modelo}\label{sec:modelo}

\subsection{Entorno y timing}
Economía de un período con un continuo de \textit{prestatarios} sin riqueza propia, $n$ \textit{bancos} idénticos con responsabilidad limitada, \textit{depositantes} con seguro de depósitos y un \textit{asegurador/fisco}. La secuencia es: (i)~la estructura de mercado queda fijada por $n$; (ii)~los bancos compiten à la Cournot eligiendo cantidades de crédito $l_i$ y el mercado determina la tasa $R_L$; (iii)~cada prestatario elige el riesgo $p$ de su proyecto; (iv)~se realiza el factor macro-financiero común $Z$, ocurren los defaults y se materializan las pérdidas.

\subsection{Prestatarios y \textit{risk-shifting}}
Cada prestatario financia con una unidad un proyecto y elige su probabilidad de default $p\in[0,1]$. Con probabilidad $1-p$ el proyecto tiene éxito y rinde $R(p)$, con $R'(p)>0$ y $(1-p)R(p)$ cóncavo. Por responsabilidad limitada repaga $1+R_L$ solo en éxito y resuelve $\max_p (1-p)[R(p)-(1+R_L)]$. La condición de primer orden $\Psi(p,R_L)\equiv(1-p)R'(p)-[R(p)-(1+R_L)]=0$, con $\Psi_p<0$, implica por el teorema de la función implícita:
\begin{equation}
...
La probabilidad de default elegida por el prestatario es estrictamente creciente en la tasa de préstamo: $p'(R_L)>0$.
\end{lemma}

La demanda inversa de crédito es $R_L=R_L(L)$, con $L=\sum_i l_i$, $R_L'(L)<0$ y $2R_L'(L)+L\,R_L''(L)<0$.

\subsection{Bancos, factor común y probabilidad de quiebra}
El default de cada préstamo se rige por un modelo de umbral con factor común $Z\sim N(0,1)$ e idiosincrásico $\varepsilon_j\sim N(0,1)$: el préstamo $j$ hace default si $\sqrt{\rho}\,Z+\sqrt{1-\rho}\,\varepsilon_j<\Phi^{-1}(p)$, donde $\rho$ es la correlación de activos. Por la ley de grandes números, la fracción de default condicional a $Z$ es la función de Vasicek
\begin{equation}
x(Z;p,\rho)=\Phi\!\left(\frac{\Phi^{-1}(p)-\sqrt{\rho}\,Z}{\sqrt{1-\rho}}\right),\qquad \partial x/\partial Z<0.
\label{eq:vasicek}
\end{equation}
Los bancos se fondean con depósitos asegurados al costo bruto $1+r_D$ y mantienen capital $e$ por unidad. El banco quiebra cuando la fracción de default supera el umbral $\bar x(R_L)=1-(1+r_D-e)/(1+R_L)$, con $\bar x'(R_L)>0$: más competencia comprime el margen, reduce $\bar x$ y acerca a la quiebra (canal Keeley). La probabilidad de quiebra individual es
\begin{equation}
\PD(R_L,p,\rho)=\Phi(\bar z)=\Phi\!\left(\frac{\Phi^{-1}(p)-\sqrt{1-\rho}\,\Phi^{-1}(\bar x(R_L))}{\sqrt{\rho}}\right).
\label{eq:pd}
\end{equation}

\subsection{Competencia à la Cournot y equilibrio}
Con responsabilidad limitada el patrimonio residual se percibe solo en supervivencia ($Z\ge\bar z$). El banco $i$ elige $l_i$ para maximizar
\begin{equation}
\Pi_i(l_i,l_{-i})=l_i\cdot\E\!\big[\max\{(1-x(Z))(1+R_L(L))-(1+r_D),0\}\big]-C(l_i).
\label{eq:profit}
\end{equation}
El prestatario responde vía Lema~\ref{lem:rs}, de modo que $p=p(R_L(L))$: la elección de riesgo es indirecta, gobernada por la tasa de equilibrio.

\begin{definition}[Equilibrio simétrico]\label{def:eq}
Un equilibrio es $(l^*,R_L^*,p^*)$ tal que: (i)~dado $R_L^*$, cada prestatario elige $p^*=p(R_L^*)$ con $\Psi(p^*,R_L^*)=0$; (ii)~$R_L^*=R_L(n\,l^*)$; (iii)~$l^*$ satisface la CPO de Cournot con $\partial^2\Pi_i/\partial l_i^2<0$; (iv)~(opcional) libre entrada fija $n^*$ por beneficio cero.
\end{definition}

\begin{proposition}[existencia y unicidad]\label{prop:exist}
Si la demanda inversa satisface las condiciones de regularidad, el beneficio \eqref{eq:profit} es estrictamente cóncavo en $l_i$ y el costo es convexo, existe un único equilibrio simétrico de Cournot. En el límite $n\to\infty$ se recupera el nivel competitivo ($R_L\to R_L^c$, margen $\to 0$) y en $n=1$ el de monopolio.
\end{proposition}

\section{Fragilidad individual y sistémica}\label{sec:sistemico}

\subsection{La relación en U (Proposición 1)}
...
\]
es decir, fragilidad sistémica y riesgo a la baja son complementarios: el efecto marginal de $\JLoss$ sobre el spread \emph{crece} cuando $\GaR$ cae (crecimiento de cola más adverso). Además la interacción se vuelve más negativa con la concentración, $\partial^3\EMBI/(\partial\JLoss\,\partial \GaR\,\partial H)<0$. (La demostración detallada paso a paso está en el Apéndice Matemático.)
\end{proposition}
...
Las predicciones no quedan solo como ejercicio deductivo: contrastadas mediante la especificación \eqref{eq:emp} sobre el panel real de economías emergentes (Sección~\ref{sec:datos_reales}), la complementariedad $\JLoss\times\GaR$ (H4a) es una regularidad robusta dentro del núcleo de economías latinoamericanas con datos más completos, y su amplificación por la concentración bancaria (H4b, la predicción que distingue a este modelo de \citealp{MMR2010}) se confirma con el signo correcto en cuatro de cinco proxies de concentración y competencia sobre el panel ampliado a once economías emergentes, con la salvedad honesta de que la magnitud todavía se estima con poder estadístico moderado. La coherencia entre el mecanismo estructural derivado en las Secciones~\ref{sec:modelo}--\ref{sec:soberano} y su huella en los datos es, en el estado actual de la evidencia, el aporte más defendible del ejercicio: no un coeficiente preciso, sino una cadena teoría--medición--evidencia que se sostiene en cada uno de sus eslabones. La agenda pendiente incluye la dinámica de dos períodos del lazo soberano-bancario (hoy de equilibrio parcial), el microfundamento completo de $\rho(n)$ más allá del caso Salop, y la ampliación del panel de economías con spread soberano observado --el camino directo para ganar la potencia estadística que hoy limita la precisión de $\hat\beta_4$.
