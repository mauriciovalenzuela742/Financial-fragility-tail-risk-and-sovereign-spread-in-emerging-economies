...
La versión no lineal de la hipótesis de amplificación se contrasta mediante un modelo de umbral (Hansen, 1999, 2000), que permite que las sensibilidades del diferencial cambien de régimen según una variable de umbral $q_{i,t}$ —en la implementación de este trabajo, el propio $GaR_{i,t}$:
...
donde $Z_{i,t}$ recoge las variables cuyo efecto se permite variar entre regímenes (en particular $JLoss_{i,t}$). Tras la transformación \textit{within} que elimina los efectos fijos, el umbral $\lambda$ se estima por mínimos cuadrados concentrados —recorriendo los valores candidatos de $q_{i,t}$ y seleccionando el que minimiza la suma de residuos al cuadrado—. La significancia del efecto umbral se evalúa mediante un contraste de razón de verosimilitud $H_0:\boldsymbol{\beta}^{(1)} = \boldsymbol{\beta}^{(2)}$, cuya distribución no estándar se aproxima por \textit{bootstrap}; el intervalo de confianza para $\lambda$ se obtiene invirtiendo el estadístico de razón de verosimilitud (Hansen, 2000). La hipótesis H3 implica que el régimen de alta vulnerabilidad ($q_{i,t}\le\lambda$) exhibe una sensibilidad del diferencial a la fragilidad estructuralmente mayor.

\subsubsection{Construcción del panel, inferencia y controles agregados}

Las series de $JLoss$, $GaR$, $FCI$ y $EMBI$ se consolidan en un único panel combinando por país y trimestre (alineación año-trimestre). Sobre el componente de los diferenciales ortogonal a los efectos fijos se aplican pruebas de raíz unitaria para panel adecuadas a paneles amplios y potencialmente desbalanceados (p. ej. el test $P_m$ de Choi, 2001). La inferencia se basa en errores estándar de Driscoll–Kraay ($vcovSCC$), robustos simultáneamente a heterocedasticidad, autocorrelación serial y dependencia transversal —justificados por los diagnósticos de sección cruzada y de autocorrelación reportados en la Sección 6.4—, y se contrastan frente a errores agrupados (\textit{clustering}) por país, por tiempo y al estimador HAC de Arellano como alternativas de robustez. Todas las variables en niveles monetarios o de diferencial admiten además una especificación en logaritmo natural, de modo que los coeficientes correspondientes se interpretan como elasticidades.

Los seis controles adicionales replicados del paper de referencia (deuda/PIB, margen de utilidad del sector bancario, volatilidad cambiaria, tasa del Tesoro de EE.UU. a 10 años, \textit{spread} de alto rendimiento y \textit{spread} on/off-the-run) se incorporan, cuando aplica, no como seis regresores sueltos sino como \textbf{dos índices agregados} —uno doméstico y uno global (Sección 6.4)—, dado que mezclar variables de variación puramente transversal-temporal con variables de variación estrictamente temporal bajo efectos fijos bidireccionales induce a que el componente global sea absorbido por los efectos de tiempo, atenuando artificialmente su poder identificador. El índice doméstico se estima con efectos fijos de país y de tiempo; el índice global, dada su naturaleza de variación puramente temporal, se estima únicamente con efectos fijos de país.
...
Panel de $\sim$19 economías emergentes a frecuencia trimestral. Cobertura temporal sujeta a la disponibilidad conjunta de insumos de balance bancario, GaR y EMBI \textit{(rango a confirmar al consolidar \texttt{Panel\_JLoss\_v7.csv})}; las tres bases descritas en la Sección 5.1 constituyen la cobertura efectivamente utilizada en esta entrega.
...
Calificación soberana (S\&P), deuda/PIB, balance fiscal/PIB, reservas/PIB, cuenta corriente/PIB, inflación interanual, tipo de cambio real efectivo y volatilidad cambiaria (controles domésticos, disponibles para el núcleo LatAm), margen de utilidad del sector bancario y PIB per cápita. Factores financieros globales: VIX, tasa del Tesoro de EE.UU. a 10 años, \textit{high-yield spread}, \textit{on/off-the-run spread}. Adicionalmente, replicando los controles del paper de referencia (Chari et al., 2024): margen de utilidad bancaria (\textit{profit margin}, reconstruido desde balances históricos 1999–2011), tasa del Tesoro de EE.UU. a 10 años, \textit{spread} de alto rendimiento (proxy ICE BofA US HY OAS) y \textit{spread} on/off-the-run construido con la curva cero-cupón de Gürkaynak-Sack-Wright de la Reserva Federal. La volatilidad cambiaria del paper de referencia queda pendiente de construcción.
...
En la base extendida (Cuadro 3), el EMBI medio de los seis países no-LatAm (216,5 pb en el agregado de 11 países) es comparable en magnitud al del núcleo LatAm, pero la fragilidad bancaria media es sustancialmente menor y más homogénea (JLoss entre 2,3 y 5,1 en los seis países nuevos, frente a la asimetría dominada por Brasil en el núcleo), un contraste relevante para interpretar el hallazgo exploratorio de la Sección 6.5.
...
\[EMBI_{i,t}=\alpha_i+\gamma_t+\beta_1 JLoss_{i,t}+\beta_2 GaR_{i,t}+\theta (JLoss\times GaR)_{i,t}+\omega' X_{i,t}+\varepsilon_{i,t}.\]
