...
\]
es decir, de \emph{signo contrario} al predicho y significativo en esa dirección bajo errores agrupados ($t=-2{,}34$), aunque el \textit{bootstrap} de bloques por país sitúa la confianza en el signo en torno al 88\% ($\Pr(\hat\beta_4>0)=12\%$). El resultado es robusto a usar la serie anual de concentración ($\hat\beta_4=-268$, $t=-2{,}14$) y a la exclusión secuencial de cada país (rango \textit{leave-one-out} $[-491,-236]$, todos negativos). Contrasta con la reconstrucción previa basada en datos regulatorios, en la que $\hat\beta_4=+721$ ($t=2{,}98$) sí confirmaba la predicción (Figura~\ref{fig:h4b}); la discrepancia se discute a continuación.
