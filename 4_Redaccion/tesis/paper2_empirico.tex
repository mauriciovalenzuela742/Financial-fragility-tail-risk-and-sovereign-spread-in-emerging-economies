...
Spread soberano medido \emph{exclusivamente} por el CDS soberano a cinco años en dólares (Bloomberg; ticker \texttt{<emisor> CDS USD SR 5Y Corp}, con la curva Markit \texttt{CCHIN1U5} para China), en puntos básicos, promediado a frecuencia trimestral. El CDS —a diferencia del EMBI de bonos— aísla el componente puro de riesgo de crédito y es homogéneo entre países. Donde no hay CDS, la observación no entra a la regresión.

\subsection{Insumos de \texorpdfstring{$JLoss$}{JLoss}}
Hojas de balance bancarias trimestrales (activos totales, patrimonio, deuda emitida, ingreso neto, ingresos, caja) y capitalización bursátil diaria de los bancos cotizados, de Bloomberg, con un protocolo de extracción común a los 113 bancos. La volatilidad del patrimonio se calcula por varianza realizada de los retornos diarios; la cotización utilizada es siempre la de la bolsa local (nunca ADR), verificada contra la moneda del balance. Motor de agregación en \texttt{jloss\_engine.py}; campos Bloomberg exactos en el Anexo~\ref{chap:anexoB}.
...
Controles domésticos: deuda del gobierno general/PIB y balance fiscal/PIB (FMI, \textit{World Economic Outlook}), reservas/PIB y cuenta corriente/PIB (Banco Mundial), inflación interanual (de la serie de IPC) y tipo de cambio real efectivo. Se reconstruyeron para \textbf{todos} los países del panel —no solo el subconjunto latinoamericano— de modo que la especificación con controles se estima sobre la muestra completa. Factores globales: VIX, tasa del Tesoro estadounidense a 10 años y \textit{spread} de alto rendimiento estadounidense (Bloomberg).
...
donde $X_{i,t}$ son los seis controles domésticos. Bajo la convención de signos adoptada, la predicción teórica del mecanismo de \textit{doom loop} es inequívoca: $\theta<0$.
