...
Este capítulo evalúa empíricamente cómo la interacción no lineal entre la fragilidad sistémica del sector bancario y los riesgos de la cola izquierda del crecimiento del producto determina el riesgo de crédito soberano en economías en desarrollo. La fragilidad bancaria se mide mediante la métrica \textit{Joint Loss} (JLoss) \citep{Chari2024b}, un indicador semiparamétrico a nivel país de la pérdida conjunta esperada del sistema bancario condicional a un evento sistémico, construido a partir de probabilidades de incumplimiento de tipo \citet{Merton1974b} agregadas vía aproximación de punto de silla \citep{MartinThompsonBrowne2001}; en esta versión, los balances bancarios y los precios accionarios que alimentan la métrica se extraen íntegramente de la terminal Bloomberg, de forma homogénea para 113 bancos de veinte economías. El riesgo a la baja del crecimiento se captura mediante \textit{Growth-at-Risk} (GaR), estimado por regresión cuantílica de panel con efectos fijos \citep{Koenker2004} sobre un índice de condiciones financieras, siguiendo el marco de \textit{vulnerable growth} de \citet{ABG2019b} y la implementación de CEMLA \citep{OssandonBusch2022}. El spread soberano se mide por el CDS soberano a cinco años (Bloomberg). Sobre el panel principal de cinco economías latinoamericanas (Brasil, Chile, Colombia, México y Perú; 2006Q4--2026Q1, $N=228$ bajo la especificación con controles), el coeficiente de interacción $JLoss_{i,t}\times GaR_{i,t}$ es negativo y significativo con controles macroeconómicos domésticos completos ($\hat\theta=-0{,}47$, $t=-2{,}15$, $p=0{,}033$), invariante al estimador de la matriz de varianzas y a la sustitución del cuantil por el \textit{expected shortfall}; a diferencia de una reconstrucción previa con datos regulatorios, el resultado \emph{depende} de la inclusión de los controles (sin ellos, $\hat\theta=-0{,}33$ no es significativo). Un modelo de umbral de panel \citep{Hansen1999,Hansen2000} corrobora la no linealidad: en el régimen de cola severa el efecto de la fragilidad sobre el spread es de $+7{,}0$ puntos básicos por unidad de JLoss, frente a $+3{,}0$ en el régimen benigno. En un panel ampliado a once economías emergentes con CDS soberano ($N=809$), el signo se mantiene con significancia estándar ($\hat\theta=-0{,}56$, $t=-2{,}25$, $p=0{,}025$), aunque el efecto se concentra en el período posterior a 2020 y desaparece al restringir la muestra al período previo. Una batería de estrategias de identificación causal (\textit{wild cluster bootstrap}, proyecciones locales, variables instrumentales por \textit{shift-share}) confirma la dirección banco$\to$soberano del canal de nivel —con un instrumento fuerte en el panel ampliado ($F=51$)—, pero muestra que la significancia puntual de la interacción es frágil bajo inferencia robusta al número reducido de \textit{clusters} de país, lo que se documenta explícitamente como límite de la inferencia antes que como debilidad oculta. Este capítulo constituye la arista empírica de la tesis; el Capítulo~\ref{chap:paper1}, de naturaleza teórica, deriva un modelo de organización industrial bancaria del cual esta complementariedad es una predicción de primer orden, y en el que se pone a prueba —con resultado negativo sobre este panel— su amplificación por la concentración del sistema bancario.
...
\begin{itemize}
    ...
    \item \textbf{Aporte (triple contribución, en la estructura de \citealp{Chari2024b}):}
    \begin{enumerate}
    ...
    \end{enumerate}
    \item \textbf{Relación con el Capítulo~\ref{chap:paper1}.} Este capítulo es la arista empírica de una tesis compuesta por dos investigaciones sobre el mismo fenómeno. El Capítulo~\ref{chap:paper1}, de naturaleza teórica, desarrolla un modelo de organización industrial bancaria en el que la estructura de mercado —la intensidad de competencia entre bancos— es el parámetro profundo que gobierna la cadena que va desde la fragilidad sistémica hasta el riesgo soberano, y del cual la complementariedad $JLoss\times GaR$ estimada aquí es una predicción de primer orden. Ese capítulo reporta, además, la prueba directa de la predicción distintiva del modelo frente a la literatura de organización industrial bancaria estándar \citep{MMR2010b}: que la complementariedad se amplifica en sistemas bancarios más concentrados. Por esa razón, la triple interacción $JLoss\times GaR\times HHI$ no se reporta en este capítulo —pertenece naturalmente al Capítulo~\ref{chap:paper1}— y el lector interesado en el mecanismo estructural completo debe remitirse a él.
...
La estrategia de construcción de datos contempla un universo objetivo de veinte economías emergentes a frecuencia trimestral. Todos los insumos micro y de mercado —balances bancarios, capitalización bursátil, CDS soberano, VIX y factores globales— provienen de una única fuente, la terminal Bloomberg, extraídos con un protocolo homogéneo entre países; solo los componentes reales del índice de condiciones financieras que alimenta $GaR$ (IPC, PIB, índice accionario, tipo de cambio real efectivo, rendimiento soberano a diez años) se mantienen de estadísticas nacionales, ya que Bloomberg no es fuente primaria de cuentas nacionales. Dado que la cobertura del CDS soberano y de los controles domésticos difiere entre países, se reporta la evidencia sobre \textbf{dos bases}:

\begin{itemize}
    \item \textbf{Base principal} — Brasil, Chile, Colombia, México y Perú; 2006Q4–2026Q1; $N=228$ observaciones efectivas bajo la especificación con controles domésticos completos (la razón deuda/PIB, disponible desde 2009Q4, es la restricción operativa; $N=285$ con los otros cinco controles, $N=363$ sin controles). Es la base sobre la que las tres métricas están más plenamente validadas y sobre la que descansa el resultado central del trabajo.
    \item \textbf{Base ampliada (11 países)} — agrega a los cinco del núcleo China, Indonesia, Malasia, Filipinas, Sudáfrica y Turquía, todas con CDS soberano de Bloomberg de historia continua 2004–2026; $N=809$ observaciones efectivas. Sin controles macroeconómicos domésticos para los seis países nuevos. Constituye la prueba de generalización del mecanismo fuera de América Latina. \textbf{Corea del Sur, que cumple el criterio de cobertura de CDS, se excluye}: el valor de mercado de sus holdings bancarios es persistentemente cercano al 4\,\% del punto de incumplimiento —frente a un rango de 15--34\,\% en el resto del panel—, reflejo del descuento estructural de valoración de la banca coreana (\textit{Korea discount}) y no de su solvencia; el modelo de Merton lee ese descuento como incumplimiento inminente y devuelve un $JLoss$ implausible (mediana 25, creciente a 47). La reconstrucción con datos regulatorios tampoco incluía a Corea. El diagnóstico completo se documenta en el repositorio.
\end{itemize}

\subsection{Variable dependiente}
Spread soberano medido por el CDS soberano a cinco años en dólares (Bloomberg), en puntos básicos, promediado a frecuencia trimestral. El CDS —a diferencia del EMBI de bonos— aísla el componente puro de riesgo de crédito y está disponible de forma homogénea para las once economías del panel.

\subsection{Insumos de $JLoss$}
Hojas de balance bancarias trimestrales (activos, patrimonio, deuda emitida, pasivos totales) y capitalización bursátil diaria de los bancos cotizados, de Bloomberg, con un protocolo de extracción común a los 113 bancos de las veinte economías. La volatilidad del patrimonio se calcula por varianza realizada de los retornos diarios; la cotización utilizada es siempre la de la bolsa local (nunca ADR), verificada contra la moneda del balance.

\subsection{Insumos de $GaR$}
Componentes del FCI por país (IPC, rendimiento soberano a 10 años, índice accionario, REER) de estadísticas nacionales; VIX de Bloomberg como proxy de condiciones financieras globales.

\subsection{Variables de control}
Deuda/PIB, balance fiscal/PIB, reservas/PIB, cuenta corriente/PIB, inflación interanual y tipo de cambio real efectivo (controles domésticos, disponibles para el núcleo de la base principal); VIX, tasa del Tesoro estadounidense a 10 años y \textit{spread} de alto rendimiento estadounidense (Bloomberg), como factores globales.

\subsection{Estadística descriptiva}

El Cuadro 2 resume las variables centrales de ambas bases. A diferencia de la reconstrucción con datos regulatorios —en la que Brasil concentraba el grueso de la fragilidad del panel (JLoss medio $\approx$16)— la serie construida homogéneamente desde Bloomberg es \textbf{mucho más comprimida en el corte transversal}: el JLoss medio por país va de 2,5 (Perú) a 4,9 (Brasil) en la base principal, y de 2,4 (Malasia) a 9,6 (Turquía) en la ampliada, con Turquía y China —no Brasil— en el extremo superior. La desviación estándar transversal del JLoss cae de 8,3 (regulatorio) a 2,5–4,1 (Bloomberg). Esta homogeneidad es una ventaja para la validez de la comparación entre países, pero —como se discute en la Sección~6.6 y en el Capítulo~\ref{chap:paper1}— reduce la variación que identifica los términos de interacción de orden superior.

\begin{center}
\footnotesize
\begin{tabular}{lrrrr}
\toprule
 & \multicolumn{2}{c}{Principal ($N=363$)} & \multicolumn{2}{c}{Ampliada ($N=809$)} \\
Variable & Media & Desv. & Media & Desv. \\
\midrule
CDS soberano 5A (pb) & 133,2 & 68,7 & 147,8 & 100,9 \\
JLoss         & 3,62   & 2,54  & 4,17   & 4,11  \\
GaR (q05)     & 0,002  & 0,035 & 0,014  & 0,040 \\
Deuda/PIB (\%)& 52,97  & 21,51 & --     & --    \\
Bal. fiscal/PIB (\%) & $-3,23$ & 2,57 & --     & --    \\
Reservas/PIB (\%)    & 18,37 & 6,90  & --     & --    \\
C. corriente/PIB (\%)& $-2,82$ & 1,81 & --     & --    \\
Inflación YoY (\%)   & 4,51 & 2,17  & --     & --    \\
REER (índice)        & 115,44 & 19,16 & --     & --    \\
\bottomrule
\end{tabular}

\medskip
\footnotesize\textbf{Cuadro 2.} Estadísticos descriptivos, base principal vs.\ ampliada (muestra efectiva de la especificación M2 sin controles). Los controles domésticos no están disponibles para los seis países agregados en la base ampliada.
\end{center}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_jloss_paises.pdf}
\caption{Serie trimestral de $JLoss$ por país, panel Bloomberg (motor de punto de silla, $\rho=0{,}4$). La escala vertical es común salvo cuando la serie del país la excede.}
\label{fig:jlosspaises}
\end{figure}

\subsection{Cobertura efectiva}

La cobertura efectiva de las regresiones es menor que la nominal: el GaR viene indefinido en los primeros trimestres de entrenamiento de cada país y la razón deuda/PIB solo está disponible desde 2009Q4, de modo que la especificación con controles completos de la base principal usa $N=228$. En la base ampliada, Malasia aporta 40 trimestres (su CDS con precios continuos arranca en 2016) y Turquía 63; el resto de los países aporta entre 66 y 89 trimestres, con una ventana que ahora se extiende hasta 2026Q1. La Figura~\ref{fig:cobertura} muestra la cobertura trimestral efectiva por país en ambas bases.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_cobertura.pdf}
\caption{Cobertura efectiva del panel: trimestres con CDS soberano, JLoss y GaR disponibles simultáneamente, por país y base.}
\label{fig:cobertura}
\end{figure}
...
\begin{tabular}{lrrrr}
\toprule
 & M2 & M3 (+controles) & cluster país & cluster tiempo \\
\midrule
\multicolumn{5}{l}{\textit{Panel A: base principal (5 LatAm, CDS + JLoss Bloomberg)}} \\
$JLoss\times GaR$ ($\theta$) & $-0,325$ & $-0,466^{**}$ & $-0,466^{***}$ & $-0,466^{***}$ \\
$t$ & $-0,89$ & $-2,15$ & $-3,37$ & $-2,63$ \\
$N$ & 363 & 228 & 228 & 228 \\
\midrule
\multicolumn{5}{l}{\textit{Panel B: base ampliada (11 países, sin controles domésticos)}} \\
$JLoss\times GaR$ ($\theta$) & $-0,557^{**}$ & --- & $-0,557^{**}$ & $-0,557^{***}$ \\
$t$ & $-2,25$ & --- & $-2,46$ & $-2,65$ \\
$N$ & 809 & --- & 809 & 809 \\
\bottomrule
\end{tabular}
\smallskip
\small\textbf{Cuadro 3.} Coeficiente de interacción $\theta$, base principal (M3 = + seis controles domésticos) vs.\ ampliada (sin controles domésticos, M2 es la especificación de referencia). Errores de Driscoll–Kraay salvo columnas de \textit{clustering}; *** $p<0{,}01$, ** $p<0{,}05$, * $p<0{,}10$. Panel anclado en Bloomberg (JLoss, CDS soberano 5A, factores globales); ver \texttt{1\_Codigo/Panel/bbg/NUMEROS\_CANONICOS\_BBG.md}.
\end{table*}

El Cuadro 3 presenta el resultado central. En M3 (base principal, con el vector completo de controles macroeconómicos domésticos), el coeficiente de interacción $\hat\theta=-0{,}466$ es negativo y significativo al 5\% ($t=-2{,}15$, $p=0{,}033$), el signo predicho por el multiplicador de \textit{doom loop}, e invariante al estimador de la matriz de varianzas (\textit{clustering} por país $t=-3{,}37$; por tiempo $t=-2{,}63$). A diferencia de la reconstrucción previa con datos regulatorios —en la que el coeficiente era prácticamente idéntico con y sin controles—, con el panel Bloomberg el resultado \textbf{depende de los controles}: en M2, sin condicionar por deuda, balance fiscal, reservas, cuenta corriente, inflación y tipo de cambio real, $\hat\theta=-0{,}325$ no alcanza significancia convencional ($t=-0{,}89$). Se reporta esta dependencia con franqueza: la complementariedad sigue siendo visible una vez saturado el modelo de fundamentos, pero la afirmación —más débil que en la versión regulatoria— es que emerge \emph{condicional a} los controles domésticos, no que sea inmune a ellos.

En la base ampliada a once economías emergentes (Panel B), la especificación de referencia M2 arroja $\hat\theta=-0{,}557$, negativo y significativo al 5\% ($t=-2{,}25$, $p=0{,}025$), con ajuste dentro del panel sano ($R^2_{\text{within}}=0{,}17$) e invariante al \textit{clustering}. La Sección~6.7 examina un matiz decisivo de este resultado: se concentra por completo en el período posterior a 2020.
...
que la Figura~\ref{fig:efmarg} traza para ambas bases. En la base principal (M3), una unidad adicional de JLoss eleva el CDS soberano en $\approx +5{,}6$ puntos básicos cuando el riesgo de cola está en su percentil 10 (cola severa), frente a $\approx +1{,}9$ en el percentil 90 (entorno benigno); en la base ampliada (M2) el gradiente va de $+9{,}8$ a $+5{,}4$. En ambos casos el efecto es monótonamente decreciente en $GaR$ —la firma empírica de la amplificación supra-aditiva—, y la banda de confianza al 90\% excluye el cero en el tramo de cola severa.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_efecto_marginal.pdf}
\caption{Efecto marginal de la fragilidad bancaria sobre el CDS soberano según el riesgo de cola del crecimiento. Banda: IC 90\% (errores Driscoll–Kraay). Eje horizontal invertido: cola más severa a la derecha.}
\label{fig:efmarg}
\end{figure}

\subsection{No linealidad: el modelo de umbral de Hansen}

Como contraste no lineal alternativo se estima un modelo de umbral de panel \citep{Hansen1999,Hansen2000}, con el propio $GaR$ como variable de umbral. En la base principal el umbral estimado es $\hat\gamma=-3{,}3$ puntos porcentuales de $GaR$: en el régimen de cola severa ($GaR\le\hat\gamma$) el efecto de la fragilidad sobre el spread es de $+7{,}0$ puntos básicos por unidad de JLoss, frente a $+3{,}0$ en el régimen benigno. En la base ampliada la brecha es aún más marcada ($+14{,}4$ vs.\ $+6{,}1$; Figura~\ref{fig:umbral}). El modelo de umbral, que no impone la forma funcional de la interacción, corrobora así el hallazgo del término multiplicativo: el efecto de la fragilidad bancaria sobre el riesgo soberano es entre dos y dos veces y media mayor cuando el crecimiento ya está en su cola izquierda.

\begin{figure}[t]
\centering
\includegraphics[width=0.75\textwidth]{fig_umbral.pdf}
\caption{Modelo de umbral de panel (Hansen): efecto de JLoss sobre el CDS soberano por régimen de $GaR$, en ambas bases.}
\label{fig:umbral}
\end{figure}

\subsection{Robustez de la base principal}

Excluyendo la pandemia (observaciones desde 2020), la interacción de la base principal se \emph{intensifica} ($\hat\theta=-1{,}13$, $t=-3{,}95$): el mecanismo de complementariedad en el núcleo latinoamericano caracteriza sobre todo la relación estructural pre-pandémica, no es un artefacto de la crisis sanitaria. Excluyendo la deuda/PIB del vector de controles —lo que amplía la muestra a $N=285$—, la interacción conserva magnitud pero pierde significancia convencional ($\hat\theta=-0{,}47$, $t=-1{,}41$), reflejo de que la identificación de M3 se apoya en la submuestra 2009Q4 en adelante donde deuda/PIB está disponible. Sustituyendo el cuantil 5\% por el \textit{Expected Shortfall} como medida de cola, la interacción conserva signo y significancia ($\hat\theta=-0{,}46$, $t=-2{,}09$).

Reestimando $\theta$ excluyendo secuencialmente cada país (\textit{leave-one-country-out}, Figura~\ref{fig:forest}, panel izquierdo), el coeficiente permanece negativo en las cinco submuestras; su magnitud es sensible sobre todo a Brasil (excluirlo lo debilita a $-0{,}23$) y a México ($-0{,}28$, $t=-1{,}79$), y excluir Perú amplía mucho el intervalo ($t=-0{,}82$). Ningún país por sí solo genera el signo, pero con una serie de fragilidad transversalmente comprimida (Cuadro 2) el margen de identificación es estrecho.
...
\subsection{Generalización fuera de LatAm: la base ampliada es un fenómeno post-2020}

La base ampliada a once economías (Panel B del Cuadro 3) arroja un $\theta$ negativo y significativo bajo inferencia estándar, y —con la serie de fragilidad homogénea de Bloomberg— el ejercicio de \textit{leave-one-country-out} es ahora limpio: $\theta$ permanece negativo al excluir cualquiera de los once países salvo China, cuya exclusión lo lleva a $-0{,}12$ ($t=-0{,}40$; Figura~\ref{fig:forest}, panel derecho). La homogenización de la métrica de fragilidad —que elimina la dominancia de Brasil que aquejaba a la reconstrucción regulatoria— resuelve, por tanto, el problema de identificación por un solo país.

Persiste, sin embargo, una limitación temporal decisiva: \textbf{el resultado de la base ampliada desaparece al restringir la muestra al período previo a 2020} ($\hat\theta=+0{,}48$, $t=+0{,}79$ con datos hasta 2019Q4). El mecanismo de complementariedad, fuera del núcleo latinoamericano, se identifica esencialmente a través de la covariación de fragilidad bancaria, riesgo de cola y spread soberano durante y después del choque pandémico y el ciclo de alzas de tasas de 2022–2023. Esto contrasta con la base principal, donde el mecanismo es \emph{más} fuerte en el período pre-pandémico. Las dos lecturas no son contradictorias —el núcleo LatAm tiene la profundidad temporal para identificar la relación estructural de largo plazo; el resto de la muestra solo tiene señal en el episodio de estrés reciente—, pero implican que la generalización del mecanismo fuera de América Latina descansa hoy sobre un único episodio macrofinanciero, y debe presentarse como tal.
...
\textbf{Wild cluster bootstrap.} Con solo cuatro (o once) clusters de país, la inferencia asintótica estándar sobre errores agrupados puede ser optimista. Al recalcular la significancia de $\theta$ mediante \textit{wild cluster bootstrap} \citep{CameronGelbachMiller2008} (999 repeticiones), el $p$-valor de la base principal es $0{,}058$ y el de la base ampliada, $0{,}141$. La significancia puntual de $\theta$ es, por tanto, \textbf{frágil} bajo un número reducido de \textit{clusters} —marginal en la base principal, no convencional en la ampliada—, un resultado que se reporta con la misma franqueza que el resto de la evidencia: lo defendible es el signo y la coherencia del mecanismo a través de especificaciones y del modelo de umbral, no una magnitud puntual con significancia a prueba de balas.

\textbf{Proyecciones locales} \citep{Jorda2005}. La respuesta dinámica del spread a un choque de fragilidad, condicionada al régimen de GaR, es ruidosa: en ambas bases el pico de respuesta en el régimen severo se ubica en el primer trimestre ($\approx+5$ puntos básicos, error estándar $1{,}1$–$1{,}5$) y no confirma limpiamente una amplificación dinámica creciente en el horizonte, lo que se reporta como diagnóstico abierto.

\textbf{Variables instrumentales por \textit{shift-share}} \citep{GoldsmithPinkhamSorkinSwift2020}. Instrumentando el nivel de $JLoss$ con choques financieros globales, se recupera un efecto de nivel positivo y de la dirección causal esperada (banco $\to$ soberano). En la base principal el instrumento (VIX) es débil ($F$ de primera etapa $=6{,}2$); en la base ampliada, el instrumento basado en el \textit{spread} on/off-run es \textbf{fuerte} ($F=51$) y el efecto de nivel de $JLoss$ sobre el CDS es positivo y marginalmente significativo ($p=0{,}08$): la mejor evidencia del canal de nivel banco$\to$soberano proviene, con datos Bloomberg, del panel ampliado.

\textbf{Triple interacción institucional.} Sustituyendo la concentración bancaria por indicadores institucionales (calidad de gobierno, WGI) en una triple interacción $JLoss\times GaR\times$Institución, el coeficiente no es significativo en ninguna base ($t=1{,}08$ y $0{,}16$).

\textbf{Síntesis de identificación causal.} Lo defendible con los datos y estrategias disponibles hoy es: (i) el mecanismo de complementariedad es teóricamente sólido y visible de forma condicional en la base principal y de forma estándar (aunque post-2020) en la ampliada; (ii) la dirección causal banco$\to$soberano en el nivel de $JLoss$ es plausible, con instrumento fuerte en el panel ampliado; (iii) la significancia \textit{puntual} de la interacción $\theta$ no sobrevive con la misma fuerza a una inferencia robusta a pocos \textit{clusters}. La contribución defendible es el signo y la coherencia del mecanismo a través de especificaciones y del modelo de umbral, en una muestra —economías emergentes con spread soberano y balances bancarios cotizados— cuya profundidad temporal por país sigue siendo el cuello de botella.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_forest_theta.pdf}
\caption{Coeficiente de interacción $\theta$ por especificación y prueba de robustez, en ambas bases. Puntos rojos: $\hat\theta>0$. Barras: IC 95\%.}
\label{fig:forest}
\end{figure}

\subsection{Diagnósticos del panel}

La prueba de Pesaran \citep{Pesaran2021} de dependencia transversal rechaza la independencia de los residuos ($p<0{,}001$) en ambas bases, lo que justifica el empleo de errores estándar de Driscoll–Kraay. La prueba de Wooldridge \citep{Wooldridge2010} detecta autocorrelación serial significativa, reforzando la necesidad de un estimador HAC. La prueba de Hausman \citep{Hausman1978} rechaza la consistencia del estimador de efectos aleatorios, confirmando la preferencia por efectos fijos. Los factores de inflación de varianza son uniformemente bajos, descartando problemas de multicolinealidad en el bloque de regresores.

\subsection{Síntesis}

La evidencia respalda la hipótesis de complementariedad en la base principal —coeficiente de interacción negativo, significativo al 5\% con el vector completo de controles domésticos, invariante al estimador de la matriz de varianzas y a la medida de cola, negativo en las cinco submuestras \textit{leave-one-out}, y corroborado por un modelo de umbral que no impone la forma funcional de la interacción—, con la salvedad de que, a diferencia de la reconstrucción regulatoria, el resultado \emph{depende} de los controles (M2 sin controles no es significativo) y el ajuste dentro del panel es débil. La base ampliada a once economías reproduce el signo con significancia estándar y sin depender de un único país, pero el efecto se concentra por completo en el período posterior a 2020. La batería de identificación causal muestra que la dirección de nivel banco$\to$soberano es defendible —con instrumento fuerte en el panel ampliado— pero que la significancia puntual de la interacción es frágil bajo inferencia robusta a pocos \textit{clusters}. Esta estructura de resultados —una complementariedad condicional y robusta a especificación en el núcleo latinoamericano, un eco reciente y estándar en el panel ampliado, y una inferencia frágil con pocos \textit{clusters}— es, en sí misma, el resumen más honesto que la evidencia actual permite ofrecer.

\section{Discusión y Conclusiones del capítulo}

\subsection{Interpretación económica}

El hallazgo central de este trabajo —un coeficiente de interacción JLoss$\times$GaR negativo, significativo al 5% condicional a los controles domésticos en la base principal, corroborado por un modelo de umbral que no impone su forma funcional— admite una interpretación económica directa en términos del mecanismo de \textit{doom loop}. La fragilidad del sistema bancario opera como una aproximación al pasivo contingente que el soberano podría verse forzado a asumir mediante un rescate; pero ese pasivo sólo se torna relevante para la prima de riesgo cuando la economía transita un estado de cola. La evidencia del modelo de umbral —un efecto de la fragilidad bancaria que pasa de negativo en entornos benignos a claramente positivo bajo estrés severo— es la firma empírica del multiplicador: no la suma de dos vulnerabilidades, sino su producto.
...
Primero, el resultado del núcleo latinoamericano descansa en cinco economías y —con datos Bloomberg— es significativo solo condicional a los controles domésticos completos, que a su vez limitan la muestra a $N=228$; la base ampliada añade profundidad transversal y una ventana hasta 2026, pero su evidencia se concentra en el episodio de estrés posterior a 2020. Segundo, la homogeneización de la métrica de fragilidad al construirla desde una única fuente —una ventaja para la comparabilidad— comprime su dispersión transversal (desviación estándar de 2,5–4,1 frente a 8,3 con datos regulatorios), lo que reduce la potencia para identificar términos de interacción de orden superior y es, como se discute en el Capítulo~\ref{chap:paper1}, la explicación más probable del resultado negativo de la prueba de amplificación por concentración. Tercero, la elección del CDS soberano a cinco años como variable dependiente —homogénea entre países, a diferencia del EMBI de bonos— aísla el riesgo de crédito puro pero introduce ruido de mercado en el núcleo LatAm, donde el mercado de bonos EMBI es más profundo; el ajuste dentro del panel de M3 en la base principal es débil. Cuarto, Corea del Sur queda fuera del panel porque el descuento estructural de valoración de su banca corrompe la lectura del modelo de Merton (Sección 5.1); otros países con descuentos de valoración menos extremos —China, con un $JLoss$ algo elevado— permanecen en la muestra y podrían compartir el sesgo en menor grado. Quinto, los controles fiscales y externos exhiben signos compatibles con endogeneidad por causalidad inversa, no corregida en este diseño. Sexto, la significancia puntual de $\theta$ es sensible al número reducido de \textit{clusters} bajo \textit{wild cluster bootstrap}, lo que exige cautela al leer los $p$-valores convencionales del Cuadro 3 como definitivos.

\subsection{Agenda futura}

De las limitaciones anteriores se desprende una agenda concreta: (i) incorporar al panel las economías con CDS soberano de Bloomberg de historia parcial (Egipto, Pakistán) y las que hoy quedan fuera por cobertura de mercado (Polonia, Hungría, Rusia, Bulgaria, con \textit{spread} de bono genérico 10Y como alternativa); (ii) reunir controles macroeconómicos domésticos para los seis países no latinoamericanos de la base ampliada, de modo de replicar la especificación M3 fuera del núcleo y verificar si el resultado post-2020 sobrevive al condicionamiento por fundamentos; (iii) extraer de Bloomberg los componentes reales del FCI (índice accionario, REER, rendimiento 10Y) para anclar también la métrica de $GaR$ en una fuente homogénea; (iv) reemplazar los datos institucionales provisionales por los \textit{Worldwide Governance Indicators} oficiales; (v) fortalecer los instrumentos de la estrategia de variables instrumentales en la base principal (shocks de política monetaria identificados, términos de intercambio por exposición sectorial); y (vi) el tratamiento formal de la dinámica y la endogeneidad mediante estimadores GMM dinámicos \citep{ArellanoBond1991}, dado el sesgo de Nickell \citep{Nickell1981}.

\subsection{Conclusión del capítulo}

Este capítulo articula, en un único marco empírico, dos literaturas que la investigación previa había mantenido separadas: la del nexo banca-soberano y la del riesgo a la baja del crecimiento. Sobre un panel construido íntegramente con datos de Bloomberg —$JLoss$ a partir de balances y precios accionarios homogéneos, spread soberano medido por el CDS a cinco años— la evidencia respalda la hipótesis central \textbf{dentro de la base principal de cinco economías latinoamericanas}, condicional al vector completo de controles domésticos: la fragilidad bancaria sistémica y el riesgo de cola del crecimiento no determinan el spread soberano de forma aditiva e independiente, sino complementaria, y el modelo de umbral corrobora la no linealidad sin imponer su forma. La base ampliada a once economías reproduce el signo con significancia estándar, si bien concentrado en el episodio de estrés posterior a 2020. La delimitación —resultado condicional en el núcleo, eco reciente en la periferia, inferencia frágil con pocos \textit{clusters}— no debilita el mecanismo sino que precisa su alcance actual. El Capítulo~\ref{chap:paper1} deriva esta misma complementariedad desde un modelo estructural de organización industrial bancaria y pone a prueba su predicción distintiva —la amplificación por concentración—, que \emph{no} encuentra respaldo en este panel. Todos los archivos computacionales de reproducción están en el repositorio, documentados en \texttt{1\_Codigo/Panel/bbg/} y trazados a \texttt{NUMEROS\_CANONICOS\_BBG.md}.
