...
Este capítulo evalúa empíricamente cómo la interacción no lineal entre la fragilidad sistémica del sector bancario y los riesgos de la cola izquierda del crecimiento del producto determina el riesgo de crédito soberano en economías en desarrollo. La fragilidad bancaria se mide mediante la métrica \textit{Joint Loss} (JLoss) \citep{Chari2024b}, un indicador semiparamétrico a nivel país de la pérdida conjunta esperada del sistema bancario condicional a un evento sistémico, construido a partir de probabilidades de incumplimiento de tipo \citet{Merton1974b} agregadas vía aproximación de punto de silla \citep{MartinThompsonBrowne2001}; en esta versión, los balances bancarios y los precios accionarios que alimentan la métrica se extraen íntegramente de la terminal Bloomberg, de forma homogénea para 113 bancos de veinte economías. El riesgo a la baja del crecimiento se captura mediante \textit{Growth-at-Risk} (GaR), estimado por regresión cuantílica de panel con efectos fijos \citep{Koenker2004} sobre un índice de condiciones financieras, siguiendo el marco de \textit{vulnerable growth} de \citet{ABG2019b} y la implementación de CEMLA \citep{OssandonBusch2022}. El spread soberano se mide por el CDS soberano a cinco años (Bloomberg). El análisis descansa sobre \textbf{un único panel} de economías emergentes: la muestra de estimación reúne todas las economías con CDS soberano, $JLoss$ y $GaR$ disponibles simultáneamente (quince países, $N=752$ bajo la especificación con controles, 2004Q1--2026Q1). El coeficiente de interacción $JLoss_{i,t}\times GaR_{i,t}$ es negativo, del signo predicho por el multiplicador de \textit{doom loop} ($\hat\theta=-0{,}30$, $t=-1{,}72$), con significancia marginal bajo errores de Driscoll--Kraay ($p=0{,}086$) y bajo \textit{wild cluster bootstrap} ($p=0{,}070$), y significancia convencional bajo \textit{clustering} por país ($p=0{,}015$). Un modelo de umbral de panel \citep{Hansen1999,Hansen2000} corrobora la no linealidad —el efecto de la fragilidad sobre el spread es $+5{,}9$ puntos básicos por unidad de $JLoss$ en el régimen de cola severa frente a $+1{,}5$ en el benigno— y el efecto marginal de la fragilidad decrece monótonamente en $GaR$. El hallazgo se reporta con una frontera explícita: \textbf{la interacción no se identifica en el período previo a 2020} ($\hat\theta=+0{,}07$, no significativo); descansa en la ventana 2020--2026 y, en particular, en el ciclo de estrés posterior a 2021. Una batería de estrategias de identificación causal (\textit{wild cluster bootstrap}, proyecciones locales, variables instrumentales por \textit{shift-share} con instrumento fuerte, $F=92$) confirma la dirección banco$\to$soberano del canal de nivel, pero muestra que la significancia puntual de la interacción es frágil bajo inferencia robusta al número reducido de \textit{clusters} de país. Este capítulo constituye la arista empírica de la tesis; el Capítulo~\ref{chap:paper1}, de naturaleza teórica, deriva un modelo de organización industrial bancaria del cual esta complementariedad es una predicción de primer orden, y en el que se pone a prueba —con resultado negativo sobre este panel— su amplificación por la concentración del sistema bancario.
...
El Cuadro 2 resume las variables centrales sobre la muestra de estimación. Un rasgo relevante para lo que sigue: a diferencia de la reconstrucción con datos regulatorios —en la que Brasil concentraba el grueso de la fragilidad del panel ($JLoss$ medio $\approx$16)— la serie construida homogéneamente desde Bloomberg es \textbf{mucho más comprimida en el corte transversal} (desviación estándar $\approx$3--4 frente a 8,3), con Turquía y China, no Brasil, en el extremo superior. Esta homogeneidad favorece la comparabilidad entre países pero, como se discute en la Sección~6.7 y en el Capítulo~\ref{chap:paper1}, reduce la variación transversal de la fragilidad que identifica los términos de interacción de orden superior.
...
\subsubsection{Frontera temporal: la interacción se identifica en el período reciente}

La estructura de resultados más importante que se debe reportar con franqueza es temporal. Restringiendo la muestra al período \textbf{previo a 2020}, la interacción conserva el signo predicho pero pierde toda precisión ($\hat\theta=-0{,}39$, $t=-1{,}02$): no se distingue de cero. En el período \textbf{2020--2026} el coeficiente es $\hat\theta=-0{,}24$ ($t=-1{,}81$), y excluyendo únicamente el bienio pandémico 2020--2021 —de modo que la muestra combina el período pre-2020 con el ciclo de alzas de tasas 2022--2026— la interacción se \textbf{intensifica} hasta $\hat\theta=-1{,}11$ ($t=-3{,}53$). La lectura es que la covariación entre fragilidad bancaria, riesgo de cola y spread soberano que identifica el mecanismo es \textbf{tenue en tiempos normales y nítida en episodios de estrés macrofinanciero}, y que el episodio 2022--2026 —el más severo del panel para economías emergentes desde 2008-09— es el que aporta la mayor parte de la señal. Esto es coherente con la naturaleza del mecanismo —un multiplicador que solo opera en la cola— pero implica que la evidencia descansa sobre un número limitado de episodios de estrés, y debe presentarse así.

\subsubsection{Robustez a especificación}

Sustituyendo el cuantil 5\% por el \textit{Expected Shortfall} como medida de cola, la interacción conserva signo y significancia ($\hat\theta=-0{,}39$, $t=-2{,}10$). Excluyendo la deuda/PIB del vector de controles la interacción se mantiene ($\hat\theta=-0{,}33$, $t=-2{,}49$). Reestimando $\theta$ excluyendo secuencialmente cada país (\textit{leave-one-country-out}, Figura~\ref{fig:forest}), el coeficiente permanece negativo en las catorce submuestras; los países más influyentes son China (excluirlo lo lleva a $-0{,}17$, $t=-1{,}33$) y Sudáfrica ($-0{,}36$, $t=-1{,}39$), mientras que excluir Turquía lo intensifica ($-0{,}41$, $t=-3{,}25$). Ningún país por sí solo genera el signo, pero con una serie de fragilidad transversalmente comprimida (Cuadro 2) el margen de identificación es estrecho.

\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{fig_forest_theta.pdf}
\caption[Coeficiente de interacción theta: especificaciones y robustez]{Coeficiente de interacción $\theta$ por especificación y prueba de robustez. Puntos rojos: $\hat\theta>0$; barras: IC 95\%. Único caso positivo: la submuestra previa a 2020 (imprecisa).}
\label{fig:forest}
\end{figure}

\subsection{Controles macroeconómicos y diagnósticos del panel}

El tipo de cambio real efectivo y la inflación entran con los signos esperados. El balance fiscal y la cuenta corriente tienden a entrar con signo contraintuitivo (mejor balance asociado a mayor spread) y la deuda/PIB con un coeficiente débil, un patrón plausiblemente atribuible a \textbf{endogeneidad por causalidad inversa}: la consolidación fiscal en estas economías tiende a producirse bajo presión de mercado. Un punto sustantivo se desprende de este contraste: la debilidad del coeficiente de \textbf{nivel} de la deuda, frente a la persistencia del signo de la interacción $JLoss\times GaR$, es coherente con la tesis del trabajo —en estas economías la vulnerabilidad soberana relevante no opera tanto a través del \textit{stock} directo de deuda cuanto a través del \textbf{pasivo contingente bancario} y su interacción con el riesgo de cola.
...
Primero, la evidencia se identifica sobre todo en episodios de estrés macrofinanciero recientes: antes de 2020 la interacción conserva el signo pero no se distingue de cero. El panel tiene amplitud transversal (catorce países) y una ventana hasta 2026, pero pocos episodios de cola independientes. Segundo, la homogeneización de la métrica de fragilidad al construirla desde una única fuente —una ventaja para la comparabilidad— comprime su dispersión transversal (desviación estándar $\approx$4 frente a 8,3 con datos regulatorios), lo que reduce la potencia para identificar términos de interacción de orden superior y es, como se discute en el Capítulo~\ref{chap:paper1}, la explicación más probable del resultado negativo de la prueba de amplificación por concentración. Tercero, once de los catorce países aportan CDS de historia larga, pero Hungría y Polonia solo tienen 14 trimestres cada uno (la ventana 2012--2015) y Pakistán apenas uno; la identificación descansa desproporcionadamente en los once países de cobertura continua. Cuarto, Corea del Sur y Bulgaria quedan fuera del panel porque su $JLoss$ no es una medida válida a nivel de país (Sección 5.1); China, con un $JLoss$ algo elevado por un descuento de valoración menos extremo, permanece en la muestra y podría compartir el sesgo en menor grado. Quinto, los controles fiscales y externos exhiben signos compatibles con endogeneidad por causalidad inversa, no corregida en este diseño. Sexto, la significancia puntual de $\theta$ es marginal —$p=0{,}056$ bajo Driscoll–Kraay, $p=0{,}035$ bajo \textit{wild cluster bootstrap}—, de modo que la contribución defendible es el signo y la forma del mecanismo, no la magnitud puntual.

\subsection{Agenda futura}

De las limitaciones anteriores se desprende una agenda concreta: (i) extender la profundidad temporal con CDS soberano de las economías que hoy solo tienen $JLoss$ y $GaR$ pero cobertura de mercado parcial (India vía bono 10Y contra el UST10Y; Egipto y Pakistán a medida que su serie de CDS se completa), y cerrar el hueco de $GaR$ para Argentina, Egipto y Rusia; (ii) extraer de Bloomberg los componentes reales del FCI (índice accionario, REER, rendimiento 10Y) para anclar también la métrica de $GaR$ en una fuente homogénea; (iii) reemplazar los datos institucionales provisionales por los \textit{Worldwide Governance Indicators} oficiales; (iv) fortalecer los instrumentos de la estrategia de variables instrumentales (shocks de política monetaria identificados, términos de intercambio por exposición sectorial) para elevar la primera etapa por encima de los umbrales convencionales; y (v) el tratamiento formal de la dinámica y la endogeneidad mediante estimadores GMM dinámicos \citep{ArellanoBond1991}, dado el sesgo de Nickell \citep{Nickell1981}.
