...
escala por un factor $\approx 2{,}5$, aunque la correlación con la serie base se mantiene en
$0{,}95$ —el ordenamiento se preserva. La métrica debe leerse, por tanto, de forma
\textbf{ordinal}; la magnitud puntual del coeficiente de interacción —ya reportada como no
interpretable— lo es aún menos en niveles, mientras que su signo, su significancia y el
patrón temporal, al depender de la estructura de correlación, son robustos a esta
recalibración (Sección~6.5).
...
\textbf{Variables instrumentales por \textit{shift-share}} \citep{GoldsmithPinkhamSorkinSwift2020}. Instrumentando el nivel de $JLoss$ con un choque financiero global interactuado con la exposición pre-muestral de cada país, el efecto de nivel instrumentado es positivo y significativo al 5\% ($p=0{,}04$), de la dirección causal esperada (banco$\to$soberano); pero la primera etapa está justo en el límite de fuerza convencional ($F\approx 9{,}5$), de modo que la evidencia es \textbf{sugestiva, no concluyente}.
