...
de esta tesis (Capítulo~\ref{chap:investigacion}), incluidas las de su subsección de agenda futura
sobre concentración bancaria (Sección~7.4). El panel se construye sobre el
...
\newcommand{\Lc}{\rightskip=0pt plus 1em\relax\arraybackslash\hyphenpenalty=50\relax\hspace{0pt}}
\begin{longtable}{%
  >{\Lc}p{2.15cm}%
  >{\Lc}p{2.7cm}%
  >{\Lc}p{1.8cm}%
...
\addlinespace
EMBI Global Diversified & Diferencial del índice de bonos soberanos emergentes de J.P.\ Morgan frente al Tesoro de EE.\ UU.\ (\textbf{DV principal}, sigue a \citealp{Chari2024b}) & J.P.\ Morgan & \texttt{embi.xlsx}; spread EMBI Global por país & Diaria & Media trimestral, pb \\
CDS soberano 5Y & \textit{Credit default swap} soberano a 5 años, USD, senior no garantizado (\textbf{robustez}) & Bloomberg & \texttt{<emisor> CDS USD SR 5Y Corp}; Markit \texttt{CCHIN1U5} para China & Diaria & Media trimestral, pb; corr.\ 0,86 con el EMBI \\
\addlinespace
...
\addlinespace
\multicolumn{6}{@{}l@{}}{\parbox{13.6cm}{\footnotesize\itshape Parámetros del motor (idénticos a \citealp{Chari2024b}): LGD $=0{,}45$; $\rho=0{,}4$; 7 nodos de cuadratura Gauss--Hermite; factor sistémico $N(0,\,0{,}16\%)$; percentil 99; cota de pérdidas $[0{,}010,\,0{,}048]$; horizonte 1 trimestre.}}\\
\addlinespace
...
Términos de intercambio (\textit{commodities}) & Choque de precios mundiales de energía, metales, materias primas agrícolas y alimentos, ponderado por la participación de exportación pre-muestra de cada país (instrumento \textit{shift-share} adicional, exógeno por diseño) & Participaciones: Banco Mundial, \textit{World Development Indicators}; precios: Banco Mundial, \textit{Pink Sheet} & \texttt{TX.\allowbreak VAL.\allowbreak \{FUEL,MMTL,AGRI,FOOD\}.\allowbreak ZS.\allowbreak UN} (1998--2003); \texttt{CMO-\allowbreak Historical-\allowbreak Data-\allowbreak Monthly.xlsx}, hoja \textit{Monthly Indices} & Anual (pesos, fijos) / mensual (precios) & Pesos pre-muestra $\times$ log del índice mundial, media trimestral \\
\addlinespace
...
bancos cotizados insuficiente— (Capítulo~\ref{chap:investigacion}, Sección~5.1). La
...
que el Capítulo~\ref{chap:investigacion} discute: (i) nueve de los trece países tienen un $JLoss$
