...
if(!exists("orth_var_dep") || !exists("orth_var_ind") || length(orth_var_dep) == 0 || length(orth_var_ind) == 0){
  orth_vars <- c()
}else{
...
for (k_date in 1:n_date) {
  ...
  for (i in 1:n_countries) {
    ...
    lead_dep_var_aux <-
      c(lead_dep_var_aux, lead(local_data[[dependent_variable]], n = h, default = NA))
  }
  ...
  for (n in 1:dim(Q_hat)[1]) {
    ...
    # observed_value <-
    #   original_my_data$g_GDP[original_my_data$Country == future_data_top$Country[n] & original_my_data$Date == selected_date_next]
    # 
    ...
    aux_support = which(PDF(x_grid) > 10 ^ -13 
                        #   abs(x_grid - min(observed_value, na.rm = TRUE)) < 2.5 |
                        #   abs(x_grid - max(observed_value, na.rm = TRUE)) < 2.5
                        )
    ...
    ES <-
      integrate(f = int,
                lower = x_grid[1],
                upper = GaR_find$root, rel.tol=.Machine$double.eps^.5)
