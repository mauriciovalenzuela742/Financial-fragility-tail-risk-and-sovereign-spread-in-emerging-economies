...
writeData(wb = wb, sheet = "Data", x = future_data)
...
addWorksheet(wb = wb,
             sheetName = "Quantile coefficients")

writeData(wb = wb, sheet = "Quantile coefficients",
          x = aux_data_frame[, c(
            'Quantile',
            'Variable',
            'Value',
...
# wb <- loadWorkbook(file = outputFile)

addWorksheet(wb = wb,
             sheetName = "Plots")

for (ind_var in union(independent_variables, orth_vars)) {
  filename = paste0("../output/images/", ind_var, ".png")
...
filename = paste0("../output/images/pseudoRsquared", ".png")
...
addWorksheet(wb = wb,
             sheetName = "Distribution results")

writeData(wb = wb, sheet = "Distribution results",
...
writeData(wb = wb, sheet = "Parameters",  x = parametersDF)
