...
get_Ryr_daily_reference <- function(dailyData, lagTilde = 2609, lagPeriod = 20,
                          lagWindow = 520, lagYear=261, originalLag = FALSE){
  n = length(dailyData$DATES)
  ...
  dailyData$chSpread <- 100 + dailyData$rRyr
  dailyData$auxMinSpread <- NA
  dailyData$CDIFF <- NA
  for(k in 1:(n-lagWindow+1)){
    dailyData$auxMinSpread[k] <- min(dailyData$chSpread[k:(k+lagWindow-1)])
  ...
  dailyData$CDIFF <- (dailyData$chSpread / dailyData$auxMinSpread) - 1
