...
def gar_from_quantiles(Q, taus, probability=0.05):
    ...
    q_upper = float(np.interp(1 - probability, taus, Q))  # cuantil 95% puntual
    ...
    # ER (Expected Longrise): media de la cola derecha desde (1-probability).
    # BUG corregido: la version anterior devolvia solo el cuantil puntual
    # Q(1-probability) en vez del promedio de la cola superior -- inconsistente
    # con ES (que si promedia) y con el metodo KDE (gar_distribution), que ya
    # promedia correctamente. Misma identidad usada para ES pero en el extremo
    # superior: E[X | X > Q(1-p)] = (1/p) * integral_{1-p}^{1} Q(tau) dtau.
    hi = tg >= (1 - probability)
    t_er = np.concatenate(([1 - probability], tg[hi]))
    q_er = np.concatenate(([q_upper], qg[hi]))
    ER = float(np.trapezoid(q_er, t_er) / probability)
